            ...
            <div class="card">
                <div class="row">
                    <div class="card-header col-md-6 col-6">
                        <h3 class="font-weight-bolder">Edit Bill</h3>
                    </div>
                ...
                <div class="card-body">
                    <form action="{{ route('admin.bill.update', $bill->id) }}" method="POST" id="form">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" id="bill_type" name="bill_type" required autocomplete="off">
                                    ...
                                    <option value="water" {{ $bill->bill_type == 'water' ? 'selected' : '' }}>Water Bill</option>
                                    <option value="internet" {{ $bill->bill_type == 'internet' ? 'selected' : '' }}>Internet Bill</option>
                                    <option value="electricity" {{ $bill->bill_type == 'electricity' ? 'selected' : '' }}>Electricity Bill</option>
                                    <option value="gas" {{ $bill->bill_type == 'gas' ? 'selected' : '' }}>Gas Bill</option>
                                    <option value="bua_moyla" {{ ($bill->bill_type == 'bua' || $bill->bill_type == 'moyla' || $bill->bill_type == 'bua_moyla') ? 'selected' : '' }}>Bua & Moyla Bill</option>
                                </select>
                            ...
                            <div class="form-group col-md-6" id="gas_cylinder_count_field" style="display: {{ $bill->bill_type == 'gas' ? 'block' : 'none' }};">
                                <label for="cylinder_count">Number of Cylinders</label>
                                <input type="number" class="form-control" id="cylinder_count" name="cylinder_count" placeholder="Enter number of cylinders" value="{{ $bill->cylinder_count ?? 1 }}" step="1" min="1" autocomplete="off">
                                @if ($errors->has('cylinder_count'))
                                    <p class="text-danger">{{ $errors->first('cylinder_count') }}</p>
                                @endif
                            </div>

                            <div class="form-group col-md-6" id="gas_cylinder_cost_field" style="display: {{ $bill->bill_type == 'gas' ? 'block' : 'none' }};">
                                <label for="cylinder_cost">Cylinder Cost (Tk.)</label>
                                <input type="number" class="form-control" id="cylinder_cost" name="cylinder_cost" placeholder="Enter cost per cylinder" value="{{ $bill->cylinder_cost ?? 1500 }}" step="0.01" min="0" autocomplete="off">
                                @if ($errors->has('cylinder_cost'))
                                    <p class="text-danger">{{ $errors->first('cylinder_cost') }}</p>
                                @endif
                            </div>

                            <div class="form-group col-md-6" id="total_amount_field" style="display: {{ $bill->bill_type == 'gas' ? 'none' : 'block' }};">
                                <label for="total_amount">Total Amount (Tk.)</label>
                                <input type="number" class="form-control" id="total_amount" name="total_amount" placeholder="Enter total bill amount" value="{{ $bill->total_amount }}" step="0.01" min="0" required autocomplete="off">
                            ...
                            <div class="form-group col-md-12" id="extra_gas_users_field" style="display: {{ $bill->bill_type == 'gas' ? 'block' : 'none' }};">
                                ...
                                <select class="form-control select2bs4" id="extra_gas_users" name="extra_gas_users[]" multiple autocomplete="off">
                                    @php
                                        $extraUsers = is_array($bill->extra_gas_users) ? $bill->extra_gas_users : [];
                                    @endphp
                                    @foreach($members as $member)
                                        <option value="{{ $member->id }}" data-member-id="{{ $member->id }}" {{ in_array($member->id, $extraUsers) ? 'selected' : '' }}>
                                            {{ $member->full_name }} 
                                            @if($member->role_id == 2)
                                                (Manager)
                                            @endif
                                    ...
                                    <div class="card-body" style="max-height: 300px; overflow-y: auto;">
                                        ...
                                        @foreach($members as $member)
                                            <div class="form-check">
                                                <input class="form-check-input applicable_member_checkbox" type="checkbox" name="applicable_member_ids[]" value="{{ $member->id }}" id="member_{{ $member->id }}" {{ in_array($member->id, $applicableMemberIds) ? 'checked' : '' }}>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Add any notes about this bill" autocomplete="off">{{ $bill->notes ?? '' }}</textarea>
                                @if ($errors->has('notes'))
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Initialize gas calculation if gas bill is selected
        ...
        // Initialize per person calculation on page load
        updatePerPersonCalculation();
        
        // Initialize extra gas users dropdown if gas bill
        if ($('#bill_type').val() === 'gas') {
            updateExtraGasUsersDropdown();
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        Toast.fire({
                            type: 'success',
                            title: response.message || 'Bill updated successfully.'
                        });
                        setTimeout(function() {
